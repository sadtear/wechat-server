...
type Config struct {
	...
	Messages MessageConfig   `yaml:"messages" json:"messages"`
}
...
// MessageConfig 公众号默认消息配置。
type MessageConfig struct {
	SubscribeReply string `yaml:"subscribe_reply" json:"subscribe_reply"` // 用户关注公众号后的自动回复内容
}
...
const defaultSubscribeReply = "欢迎关注 AI80！\n\n" +
...
func defaultConfig() *Config {
	return &Config{
		...
		Messages: MessageConfig{
			SubscribeReply: defaultSubscribeReply,
		},
	}
}
...
func applyEnvOverrides(target *Config) {
	...
	if subscribeReply := os.Getenv("WECHAT_SUBSCRIBE_REPLY"); subscribeReply != "" {
		target.Messages.SubscribeReply = normalizeEnvText(subscribeReply)
	}
...
func normalizeConfig(target *Config) {
	...
	target.Messages.SubscribeReply = strings.TrimSpace(target.Messages.SubscribeReply)
	if target.Messages.SubscribeReply == "" {
		target.Messages.SubscribeReply = defaultSubscribeReply
	}
}
...
func normalizeEnvText(value string) string {
